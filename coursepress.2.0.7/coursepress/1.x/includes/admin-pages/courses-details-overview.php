				...
				<?php if ( 0 == $course_id && CoursePress_Capabilities::can_create_course() || CoursePress_Capabilities::can_update_course( $course_id ) ) { ?>
					...
					<div id='edit-sub' class='course-holder-wrap mp-wrap'>
						...
						<div class='sidebar-name no-movecursor'>
							<h3><?php _e( 'Course Setup', 'coursepress' ); ?></h3>
						</div>
						...
						<div class='course-holder'>
							...
							<div class="unit-control-buttons course-control-buttons">
								...
								<?php /* if (( $course_id == 0 && current_user_can('coursepress_create_course_cap'))) {//do not show anything
								  ?>
								  <input type="submit" name="submit-unit" class="button button-units save-unit-button" value="<?php _e('Save Draft', 'coursepress'); ?>">
								  <input type="submit" name="submit-unit-publish" class="button button-units button-publish" value="<?php _e('Publish', 'coursepress'); ?>">
								...
								<?php /* if (( $course_id != 0 && current_user_can('coursepress_update_course_cap') ) || ( $course_id != 0 && current_user_can('coursepress_update_my_course_cap') && $course_details->post_author == get_current_user_id() )) {//do not show anything
								  ?>
								  <input type="submit" name="submit-unit" class="button button-units save-unit-button" value="<?php echo ( $course_details->post_status == 'unpublished' ) ? __('Save Draft', 'coursepress') : __('Publish', 'coursepress'); ?>">
								  <?php } */ ?>

								<?php
								if ( $course_id != 0 && CoursePress_Capabilities::can_update_course( $course_id ) ) {//do not show anything
									?>
									<a class="button button-preview-overview" href="<?php echo get_permalink( $course_id ); ?>" target="_new"><?php _e( 'Preview', 'coursepress' ); ?></a>
									...
									/* if (current_user_can('coursepress_change_course_status_cap') || ( current_user_can('coursepress_change_my_course_status_cap') && $course_details->post_author == get_current_user_id() )) { ?>
									  <input type="submit" name="submit-unit-<?php echo ( $course_details->post_status == 'unpublished' ) ? 'publish' : 'unpublish'; ?>" class="button button-units button-<?php echo ( $course_details->post_status == 'unpublished' ) ? 'publish' : 'unpublish'; ?>" value="<?php echo ( $course_details->post_status == 'unpublished' ) ? __('Publish', 'coursepress') : __('Unpublish', 'coursepress'); ?>">
									  <?php
									  } */
								}
								?>
								...
								if ( ( isset( $_GET['course_id'] ) ) || ! isset( $_GET['course_id'] ) && $not_limited ) {
									...
									<div class="course-section step step-1 <?php echo 'step-1' == $course_setup_marker ? 'save-marker active' : ''; ?>">
										<div class='course-section-title'>
											...
											<h3><?php _e( 'Step 1 - Course Overview', 'coursepress' ) ?></h3>
										</div>
										<div class='course-form'>
											...
											<div class="wide">
												<label for='course_name' class="required">
													<?php _e( 'Course Name', 'coursepress' ); ?>
												</label>
											...
											<div class="wide">
												<label for='course_excerpt' class="required">
													<?php _e( 'Course Excerpt / Short Overview', 'coursepress' ); ?>
													<?php //CP_Helper_Tooltip::tooltip( __( 'Provide a few short sentences to describe the course', 'coursepress' ) );    ?>
												</label>
												<?php
											...
											<div class="wide narrow">
												<label for='featured_url'>
													<?php _e( 'Listing Image', 'coursepress' ); ?><br/>
													<span><?php _e( 'The image is used on the "Courses" listing ( archive ) page along with the course excerpt.', 'coursepress' ) ?></span>
												</label>
												...
												<div class="featured_url_holder">
													...
													?>" placeholder="<?php _e( 'Add Image URL or Browse for Image', 'coursepress' ); ?>"/>
													<input class="featured_url_button button-secondary" type="button" value="<?php _e( 'Browse', 'coursepress' ); ?>"/>
													...
													?>
													<div class="invalid_extension_message"><?php echo sprintf( __( 'Extension of the file is not valid. Please use one of the following: %s', 'coursepress' ), $supported_image_extensions ); ?></div>
												</div>
											</div>
											...
											<div class="narrow">
												<label>
													<?php _e( 'Course Category', 'coursepress' ); ?>
													<a class="context-link" href="edit-tags.php?taxonomy=course_category&post_type=course"><?php _e( 'Manage Categories', 'coursepress' ); ?></a>
												</label>
												<?php
												$x = '';
											...
											<div class="narrow">
												<label for='meta_course_language'><?php _e( 'Course Language', 'coursepress' ); ?></label>
											...
											<div class="course-step-buttons">
												<input type="button" class="button button-units next" value="<?php _e( 'Next', 'coursepress' ); ?>"/>
												<input type="button" class="button button-units update" value="<?php _e( 'Update', 'coursepress' ); ?>"/>
											</div>
										</div>
									</div>
									<!-- /Course Overview -->
									...
									<div class="course-section step step-2 <?php echo 'step-2' == $course_setup_marker ? 'save-marker active' : ''; ?>">
										<div class='course-section-title'>
											...
											<h3><?php _e( 'Step 2 - Course Description', 'coursepress' ) ?></h3>
										</div>
										<div class='course-form'>
											<?php
											...
											<div class="wide narrow">
												...
												<div class="video_url_holder mp-wrap">
													<label for='meta_course_video_url'>
														<?php _e( 'Featured Video', 'coursepress' ); ?><br/>
														<span><?php _e( 'This is used on the Course Overview page and will be displayed with the course description.', 'coursepress' ); ?></span>
													</label>
													...
													_e( 'Add URL or Browse', 'coursepress' );
													echo ' ( ' . $supported_video_extensions . ' )';
													?>"/>

													<input type="button" class="course_video_url_button button-secondary" value="<?php _e( 'Browse', 'coursepress' ); ?>"/>
											...
											<div class="wide">
												<label for='course_description' class="required">
													<?php _e( 'Course Description', 'coursepress' ); ?>
													<?php // CP_Helper_Tooltip::tooltip( __( 'Provide a detailed description of the course', 'coursepress' ) );       ?>
												</label>

												<p><?php _e( 'This is an in-depth description of the course. It should include such things like an overview, outcomes, possible requirements, etc.', 'coursepress' ); ?></p>
												<?php
											...
											<div class="wide">
												<label>
													...
													<?php // CP_Helper_Tooltip::tooltip( __( 'Provide a detailed description of the course', 'coursepress' ) );       ?>
													<br/>
													<span><?php _e( 'This gives you the option to show/hide Course Units, Lessons, Estimated Time and Free Preview options on the Course Overview page', 'coursepress' ); ?></span>
												</label>
												...
												<div class="course-structure">
													...
													<label for="meta_course_structure_options"><?php _e( 'Show the Course Overview structure and Preview Options', 'coursepress' ); ?></label><br/>
													...
													<label for="meta_course_structure_time_display"><?php _e( 'Display Time Estimates for Units and Lessons', 'coursepress' ); ?></label>
													<table>
														<thead>
															<tr>
																...
																<th class="column-time"><?php _e( 'Time', 'coursepress' ); ?></th>
															</tr>
															<tr class="break">
																<td colspan="4"></td>
															</tr>
														...
														<tbody>
															<tr>
																<td colspan="4">
																	<ol class="tree">
																		<li>
																			<label for="course_<?php echo ( ! isset( $course ) || ! empty( $course->details ) ) ? $course->details->ID : '0'; ?>"><?php echo( ! isset( $course ) || ! empty( $course->details ) && $course->details->post_title && $course->details->post_title !== '' ? $course->details->post_title : __( 'Course', 'coursepress' ) ); ?></label>
																			...
																			if ( 0 == count( $units ) ) {
																				...
																				<ol>
																					<li>
																						<label><?php _e( 'There are currently no units to display', 'coursepress' ); ?></label>
																					</li>
																				</ol>
																			<?php } else {
																				?>
																					...
																					foreach ( $units as $unit ) {
																						...
																						<li class="<?php echo( $unit_post->post_status == 'publish' ? 'enabled_unit' : 'disabled_unit' ); ?>">
																							...
																							<label for="unit_<?php echo $unit_id; ?>">
																								<div class="tree-unit-left"><?php echo( $unit_post->post_status != 'publish' ? __( '[draft] ', 'coursepress' ) : '' ); ?><?php echo $unit_post->post_title; ?></div>
																							...
																							<ol>
																								...
																								if ( $unit_pages == 0 ) {
																									?>
																									<li>
																										<label><?php _e( 'There are currently no pages to display', 'coursepress' ); ?></label>
																									</li>
																								<?php
																								} else {
																									?>
																										...
																										for ( $i = 1; $i <= $unit_pages; $i ++ ) {
																											...
																											<label for="page_<?php echo $unit_id . '_' . $i; ?>">
																												<div class="tree-page-left">
																													<?php echo( isset( $page_title ) && $page_title !== '' ? $page_title : __( 'Untitled Page', 'coursepress' ) ); ?>
																												</div>
																											...
																											<ol class="course_structure_elements_ol">
																												...
																												/*
																												  ...
																												  <?php echo ($mod->post_title && $mod->post_title !== '' ? $mod->post_title : __('Untitled Element', 'coursepress')); ?>
																												  </div>
																												  ...
																												  <span><?php echo (isset($mod->time_estimation) && $mod->time_estimation !== '') ? $mod->time_estimation.' '.__('min', 'coursepress') : __('N/A', 'coursepress');?></span>
																												  </div>
																												  </li>
																												  <?php
																												  }
															...
															/* $units = $course->get_units();
															  ...
															  ?>
															  <tr>
															  <th colspan="4"><?php _e('There are currently no Units to Display', 'coursepress'); ?></th>
															  </tr>
															  <?php
															  } else { */
															/* foreach ($units as $unit) {
											...
											<div class="course-step-buttons">
												<input type="button" class="button button-units prev" value="<?php _e( 'Previous', 'coursepress' ); ?>"/>
												<input type="button" class="button button-units next" value="<?php _e( 'Next', 'coursepress' ); ?>"/>
												<input type="button" class="button button-units update" value="<?php _e( 'Update', 'coursepress' ); ?>"/>
											</div>
										</div>
									...
									<div class="course-section step step-3 <?php echo 'step-3' == $course_setup_marker ? 'save-marker active' : ''; ?>">
										<div class='course-section-title'>
											...
											<h3><?php _e( 'Step 3 - Instructors', 'coursepress' ) ?></h3>
										</div>
										<div class='course-form'>
											<?php
											...
											<div class="wide narrow">
												<label>
													<?php _e( 'Course Instructor(s)', 'coursepress' ); ?>
													<?php // CP_Helper_Tooltip::tooltip( __( 'Select one or more instructor to facilitate this course.', 'coursepress' ) );                 ?>
													<br/>
													<span><?php _e( 'Select one or more instructor to facilitate this course', 'coursepress' ); ?></span>
												</label>
												...
												<?php if ( CoursePress_Capabilities::can_assign_course_instructor( $course_id ) ) { ?>
													...
													<input class="button-primary" id="add-instructor-trigger" type="button" value="<?php _e( 'Assign', 'coursepress' ); ?>">
													<!-- <p><?php _e( 'NOTE: If you need to add an instructor that is not on the list, please finish creating your course and save it. To create a new instructor, you must go to Users to create a new user account which you can select in this list. Then come back to this course and you can then select the instructor.', 'coursepress' ); ?></p> -->
												...
												<?php
													...
													if ( cp_get_number_of_instructors() == 0 || cp_instructors_avatars( $course_id, false, true ) == 0 ) {//just to fill in emtpy space if none of the instructors has been assigned to the course and in the same time instructor can't assign instructors to a course
														_e( 'You do not have required permissions to assign instructors to a course.', 'coursepress' );
													}
												}
												?>

												<p><?php _e( 'Assigned Instructors:', 'coursepress' ); ?></p>
												...
												<div class="instructors-info" id="instructors-info">
													<?php if ( 0 >= cp_instructors_avatars( $course_id, true, true ) ) : ?>
														<div class="instructor-avatar-holder empty">
															<span class="instructor-name"><?php _e( 'Please Assign Instructor', 'coursepress' ); ?></span>
														</div>
													<?php endif ?>
												...
												<?php if ( $can_manage_instructors || 0 == $course_id ) : ?>
													...
													<label>
														<?php _e( 'Invite New Instructor', 'coursepress' ); ?>
														<?php // CP_Helper_Tooltip::tooltip( __( 'If the instructor can not be found in the list above, you will need to invite them via email.', 'coursepress' ) );                 ?>
														<br/>
														<span><?php _e( 'If the instructor can not be found in the list above, you will need to invite them via email.', 'coursepress' ); ?></span>
													</label>
													<div class="instructor-invite">
														<label for="invite_instructor_first_name"><?php _e( 'First Name', 'coursepress' ); ?></label>
														<input type="text" name="invite_instructor_first_name" placeholder="<?php _e( 'First Name', 'coursepress' ); ?>"/>
														<label for="invite_instructor_last_name"><?php _e( 'Last Name', 'coursepress' ); ?></label>
														<input type="text" name="invite_instructor_last_name" placeholder="<?php _e( 'Last Name', 'coursepress' ); ?>"/>
														<label for="invite_instructor_email"><?php _e( 'E-Mail', 'coursepress' ); ?></label>
														<input type="text" name="invite_instructor_email" placeholder="<?php _e( 'instructor@email.com', 'coursepress' ); ?>"/>

														<div class="submit-message">
															<input class="button-primary" name="invite_instructor_trigger" id="invite-instructor-trigger" type="button" value="<?php _e( 'Send Invite', 'coursepress' ); ?>">
														</div>
													</div>
												<?php endif; ?>
									...
									<div class="course-section step step-4 <?php echo 'step-4' == $course_setup_marker ? 'save-marker active' : ''; ?>">
										<div class='course-section-title'>
											...
											<h3><?php _e( 'Step 4 - Course Dates', 'coursepress' ) ?></h3>
										</div>
										<div class='course-form'>
											<?php
											...
											<div class="wide course-dates">
												<label>
													<?php _e( 'Course Dates', 'coursepress' ); ?>
													<?php // CP_Helper_Tooltip::tooltip( __( 'This is the duration the course will be open to the students.', 'coursepress' ) );                 ?>
												</label>

												<div class="course-date-override">
													<label><input type="checkbox" name="meta_open_ended_course" id="open_ended_course" <?php echo ( $open_ended_course == 'on' ) ? 'checked' : ''; ?> /><?php _e( 'This course has no end date', 'coursepress' ); ?>
													</label>
												</div>

												<p><?php _e( 'This is the duration the course will be open to the students', 'coursepress' ); ?></p>

												<div class="date-range">
													<div class="start-date">
														<label for="meta_course_start_date" class="start-date-label required"><?php _e( 'Start Date', 'coursepress' ); ?></label>
													...
													<div class="end-date <?php echo ( $open_ended_course == 'on' ) ? 'disabled' : ''; ?>">
														<label for="meta_course_end_date" class="end-date-label <?php echo ( $open_ended_course == 'on' ) ? '' : 'required'; ?>"><?php _e( 'End Date', 'coursepress' ); ?></label>
											...
											<div class="wide enrollment-dates">
												<label>
													<?php _e( 'Enrollment Dates', 'coursepress' ); ?>
													<?php // CP_Helper_Tooltip::tooltip( __( 'These are the dates that students can enroll.', 'coursepress' ) );                 ?>
												</label>

												<div class="enrollment-date-override">
													<label><input type="checkbox" name="meta_open_ended_enrollment" id="open_ended_enrollment" <?php echo ( $open_ended_enrollment == 'on' ) ? 'checked' : ''; ?> /><?php _e( 'Users can enroll at any time', 'coursepress' ); ?>
													</label>
												</div>

												<p><?php _e( 'These are the dates that students can enroll', 'coursepress' ); ?></p>

												<div class="date-range">
													<div class="start-date <?php echo ( $open_ended_enrollment == 'on' ) ? 'disabled' : ''; ?>">
														<label for="meta_enrollment_start_date" class="start-date-label <?php echo ( $open_ended_enrollment == 'on' ) ? '' : 'required'; ?>"><?php _e( 'Start Date', 'coursepress' ); ?></label>
													...
													<div class="end-date <?php echo ( $open_ended_enrollment == 'on' ) ? 'disabled' : ''; ?>">
														<label for="meta_enrollment_end_date" class="end-date-label <?php echo ( $open_ended_enrollment == 'on' ) ? '' : 'required'; ?>"><?php _e( 'End Date', 'coursepress' ); ?></label>
									...
									<div class="course-section step step-5 <?php echo 'step-5' == $course_setup_marker ? 'save-marker active' : ''; ?>">
										<div class='course-section-title'>
											...
											<h3><?php _e( 'Step 5 - Classes, Discussion & Workbook', 'coursepress' ) ?></h3>
										</div>
										<div class='course-form'>
											<?php
											...
											<div class="wide narrow">
												<div>
													<label for='meta_class-size'>
														...
														<?php // CP_Helper_Tooltip::tooltip( __( 'Use this setting to set a limit for all classes. Uncheck for unlimited class size( s ).', 'coursepress' ) );                    ?>
														<br/>
														<span><?php _e( 'Use this setting to set a limit for all classes. Uncheck for unlimited class size( s ).', 'coursepress' ); ?></span>
													</label>
												...
												<label for='meta_allow_course_discussion'>
													...
													<span><?php _e( 'Allow Course Discussion', 'coursepress' ); ?></span>
													<?php // CP_Helper_Tooltip::tooltip( __( 'If checked, students can post questions and receive answers at a course level. A \'Discusssion\' menu item is added for the student to see ALL discussions occuring from all class members and instructors.', 'coursepress' ) );                    ?>
													<br/>
													<span><?php _e( 'If checked, students can post questions and receive answers at a course level. A \'Discusssion\' menu item is added for the student to see ALL discussions occuring from all class members and instructors.', 'coursepress' ); ?></span>
												</label>
												...
												<label for='meta_allow_workbook_page'>
													...
													<span><?php _e( 'Show student Workbook', 'coursepress' ); ?></span>
													<?php // CP_Helper_Tooltip::tooltip( __( 'If checked, students can see their progress and grades.', 'coursepress' ) );                    ?>
													<br/>
													<span><?php _e( 'If checked, students can see their progress and grades.', 'coursepress' ); ?></span>
												</label>
									...
									<div class="course-section step step-6 <?php echo 'step-6' == $course_setup_marker ? 'save-marker active' : ''; ?>">
										<div class='course-section-title'>
											...
											<?php
											$section_title = __( 'Step 6 - Enrollment & Course Cost', 'coursepress' );
											if ( ! $offer_paid ) {
												$section_title = __( 'Step 6 - Enrollment', 'coursepress' );
											}
											?>
										...
										<div class='course-form'>
											...
											<div class="narrow">
												<label for='meta_enroll_type'>
													<?php _e( 'Who can Enroll in this course', 'coursepress' ); ?>
													<?php // CP_Helper_Tooltip::tooltip( __( 'Select the limitations on accessing and enrolling in this course.', 'coursepress' ) );                  ?>
													<br/>
													<span><?php _e( 'Select the limitations on accessing and enrolling in this course.', 'coursepress' ); ?></span>
												</label>
												...
												<?php //_e( 'In order to allow course enrollment (other than Manually) you have to activate "Anyone can register" from the WordPress settings.', 'coursepress' );     ?><!--</span>-->
												<?php //} ?>
											</div>

											<div class='wide' id='manually_added_holder'>
												<p><?php _e( 'NOTE: If you need to manually add a student, students must be registered on your site first. To do this for a student, you can do this yourself by going to Users in WordPress where you can add the students manually. You can then select them from this list.', 'coursepress' ); ?></p>
											</div>

											<div class="wide" id="enroll_type_prerequisite_holder" <?php echo( $enroll_type <> 'prerequisite' ? 'style="display:none"' : '' ) ?>>
												<label for='meta_enroll_type'>
													<?php _e( 'Prerequisite Course', 'coursepress' ); ?>
													<?php // CP_Helper_Tooltip::tooltip( __( 'Students will need to fulfil prerequisite in order to enroll.', 'coursepress' ) );                  ?>
												</label>

												<p><?php _e( 'Students will need to complete the following prerequisite course in order to enroll.', 'coursepress' ); ?></p>
											...
											<div class="narrow" id="enroll_type_holder" <?php echo( $enroll_type <> 'passcode' ? 'style="display:none"' : '' ) ?>>
												<label for='meta_enroll_type'>
													<?php _e( 'Pass Code', 'coursepress' ); ?>
													<?php // CP_Helper_Tooltip::tooltip( __( 'Students will need to enter this pass code in order to enroll.', 'coursepress' ) );                  ?>
												</label>

												<p><?php _e( 'Students will need to enter this pass code in order to enroll.', 'coursepress' ); ?></p>
											...
											if ( $offer_paid ) {
												if ( cp_use_woo() ) {
													...
													<div class="narrow product">
														...
														<label>
															<?php _e( 'Cost to participate in this course', 'coursepress' ); ?>
														</label>
														...
														<div class="course-paid" id="marketpressprompt">
															...
															<span><?php _e( 'This is a Paid Course', 'coursepress' ); ?></span>
														</div>
															...
															<div class="course-paid-course-details <?php echo ( $paid_course != 'on' ) ? 'hidden' : ''; ?>">
																<div class="course-sku">
																	<p>
																		...
																		<?php _e( 'Automatically generate Stock Keeping Unit (SKU)', 'coursepress' ); ?>
																	</p>
																...
																<div class="course-price">
																	<span class="price-label <?php echo $paid_course == 'on' ? 'required' : ''; ?>"><?php _e( 'Price', 'coursepress' ); ?></span>
																...
																<div class="course-sale-price">
																	...
																	<p>
																		...
																		<?php _e( 'Enabled Sale Price', 'coursepress' ); ?></p>
																	<span class="price-label <?php isset( $woo_product_details ) && ! empty( $woo_product_details["_sale_price"] ) && checked( $woo_product_details["_sale_price"][0], '1' ) ? 'required' : ''; ?>"><?php _e( 'Sale Price', 'coursepress' ); ?></span>
													...
													<div class="narrow product">
														...
														if ( current_user_can( 'install_plugins' ) && current_user_can( 'activate_plugins' ) ) {
															...
															<div class="cp-markertpress-not-active <?php echo CoursePress_MarketPress_Integration::is_active() ? 'hidden' : ''; ?>">
																<div id="marketpressprompt-box">
																	<label>
																		<?php _e( 'Sell your courses online with MarketPress.', 'coursepress' ); ?>
																	</label>
																	...
																	if ( ! CoursePress_Capabilities::is_pro() ) {
																		echo sprintf( __(
																			...
																			'If you require other payment gateways, you will need to upgrade to %s.', 'coursepress' ), '<a target="_blank" href="' . admin_url( 'admin.php?page=' . $this->screen_base . '_settings' . '&tab=cp-marketpress' ) . '">' . __( 'Begin Activating MarketPress Lite', 'coursepress' ) . '</a>', '<a target="_blank" href="https://premium.wpmudev.org/project/e-commerce/">' . __( 'MarketPress', 'coursepress' ) . '</a>' );
																	} else {
																		echo sprintf( __( 'The full version of MarketPress has been bundled with %s.<br />' .
																		                  'To start selling your course, you will need to activate MarketPress: <br /> %s<br /><br />', 'coursepress' ), 'CoursePress', '<a target="_blank" href="' . admin_url( 'admin.php?page=' . $this->screen_base . '_settings' . '&tab=cp-marketpress' ) . '">' . __( 'Begin Activating MarketPress', 'coursepress' ) . '</a>' );
																	}
																	?>
																</div>
														...
														<!--_e('Please ask administrator to enable at least one payment gateway.', 'coursepress');-->
													</div>
											...
											<div class="course-step-buttons">
												<input type="button" class="button button-units prev" value="<?php _e( 'Previous', 'coursepress' ); ?>"/>
												<input type="button" class="button button-units update" value="<?php _e( 'Update', 'coursepress' ); ?>"/>
												<input type="button" class="button button-units done" value="<?php _e( 'Done', 'coursepress' ); ?>"/>
											</div>
										</div>
									</div>
									<!-- /Enrollment & Course Cost -->
									...
									<!-- OLD GRADEBOOK INTEGRATION
																									...
																									<label><?php _e( 'Show Grades Page for Students', 'coursepress' ); ?>
																											<a class="help-icon" href="javascript:;"></a>
																											<div class="tooltip">
																													<div class="tooltip-before"></div>
																													<div class="tooltip-button">&times;</div>
																													<div class="tooltip-content">
									<?php _e( 'If checked, students can see their course performance and grades by units.', 'coursepress' ) ?>
																													</div>
																											</div>
						...
						if ( $course_id !== 0 ) {
							?>
																																																				<a href="<?php echo admin_url( 'admin.php?page=' . (int) $_GET['page'] . '&tab=units&course_id=' . (int) $_GET['course_id'] ); ?>" class="button-secondary"><?php _e( 'Add Units &raquo;', 'coursepress' ); ?></a>
						<?php } ?>
						</div>
						-->
