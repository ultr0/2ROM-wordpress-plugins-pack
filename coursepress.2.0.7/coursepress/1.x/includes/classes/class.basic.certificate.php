	...
	class CP_Basic_Certificate {
		...
		public static function add_settings_item( $menus ) {
			...
			foreach( $menus as $key => $value ) {
				if( $key != $insert_before ) {
					...
					if( ! $inserted) {
						$new_menu['basic_certificate'] = __( 'Basic Certificate', 'coursepress' );
						$inserted = true;
					}
					$new_menu[ $key ] = $value;
				}
		...
		public static function render_settings() {
			...
			<form action='' method='post'>
				...
				<div class="postbox">
					<h3 class="hndle" style='cursor:auto;'><span><?php _e( 'Certificate Options', 'coursepress' ); ?></span></h3>
					<div class="inside">
						<table class="form-table">
							<tbody>
							<tr valign="top">
								<th scope="row">
									<?php _e( 'Enable Basic Certificate', 'coursepress' ); ?>
									<a class="help-icon" href="javascript:;"></a>
										<div class="tooltip">
											<div class="tooltip-before"></div>
											<div class="tooltip-button">&times;</div>
											<div class="tooltip-content">
												<?php _e( 'Adds a "Certificate" link to completed courses on the student dashboard.', 'coursepress' ); ?>
											</div>
										</div>
								</th>
								<td>
							...
							<!--<tr valign="top">-->
								<!--<th scope="row">--><?php //_e( 'Email certificate when course is completed.', 'coursepress' ); ?><!--</th>-->
								<!--<td>-->
				...
				<div class="postbox">
					<h3 class="hndle" style='cursor:auto;'><span><?php _e( 'Certificate Layout', 'coursepress' ); ?></span></h3>

					<div class="inside">
						<p class="description"><?php _e( 'Use the editor below to create the layout of your certificate.', 'coursepress' ); ?></p>
						<p class="description"><?php _e( 'These codes will be replaced with actual data: FIRST_NAME, LAST_NAME, COURSE_NAME, COMPLETION_DATE, CERTIFICATE_NUMBER, UNIT_LIST', 'coursepress' ); ?></p>
						<table class="form-table">
							<tbody id="items">
								<tr>
									<td>
								...
								<tr>
									<td>
										...
										<strong><?php esc_html_e( 'Background Image', 'coursepress' ); ?></strong>
										<a class="help-icon" href="javascript:;"></a>
										<div class="tooltip">
											<div class="tooltip-before"></div>
											<div class="tooltip-button">&times;</div>
											<div class="tooltip-content">
												<?php echo sprintf( __( 'The image will be resized to fit the full page. For best results use 1:1414 as the ratio for image dimensions.<br /><br /><strong>Examples:</strong><br />595x842px (Portrait 72dpi)<br />1754x1240px (Landscape 150dpi)<br />2480x3508px (Portrait 300dpi).', 'coursepress' ) ) ; ?>
											</div>
										</div><br />
										<?php
										...
										<div class="certificate_background_image_holder">
											...
											<input class="certificate_background_button button-secondary" type="button" value="<?php _e( 'Browse', 'coursepress' ); ?>"/>
								...
								<tr>
									<td>
										<p>
											<strong><?php esc_html_e( 'Content Padding', 'coursepress' ); ?></strong>
											<span class="description"><?php esc_html_e( 'Can be any CSS units. E.g. "0.2em"', 'coursepress'); ?></span>
										</p>

										<span><?php esc_html_e( 'Top', 'coursepress' ) ?></span><input type="text" size="6" style="width: 80px;" class="padding_top" name="cert_field_padding_top" value="<?php echo esc_html( self::option('padding_top') ); ?>" />
										<span><?php esc_html_e( 'Bottom', 'coursepress' ) ?></span><input type="text" size="6" style="width: 80px;" class="padding_bottom" name="cert_field_padding_bottom" value="<?php echo esc_html( self::option('padding_bottom') ); ?>" />
										<span><?php esc_html_e( 'Left', 'coursepress' ) ?></span><input type="text" size="6" style="width: 80px;" class="padding_left" name="cert_field_padding_left" value="<?php echo esc_html( self::option('padding_left') ); ?>" />
										<span><?php esc_html_e( 'Right', 'coursepress' ) ?></span><input type="text" size="6" style="width: 80px;" class="padding_right" name="cert_field_padding_right" value="<?php echo esc_html( self::option('padding_right') ); ?>" />
									</td>
								</tr>
								<tr>
									<td>
										<strong><?php esc_html_e( 'Page Orientation', 'coursepress' ); ?></strong><br />
										...
										<select name="cert_field_orientation" style="width: max-width: 200px;" id="cert_field_orientation">
											<option value="L" <?php selected( $selected_type, 'L', true ); ?>><?php _e( 'Landscape', 'coursepress' ); ?></option>
											<option value="P" <?php selected( $selected_type, 'P', true ); ?>><?php _e( 'Portrait', 'coursepress' ); ?></option>
										</select>
									</td>
								</tr>
							</tbody>
		...
		private static function render_css_setting() {
			?>
			<div class="postbox">
				<h3 class="hndle" style='cursor:auto;'><span><?php _e( 'CSS', 'coursepress' ); ?></span></h3>
				<div class="inside">
					<p class="description"><?php _e( 'You can specify individual CSS rules for each of the certificate fields.<br />The follow CSS classes are available: .first_name, .last_name, .course_name, .completion_date, .certificate_number, .unit_list.<br />They are all wrapped in the .basic_certificate DIV tag.', 'coursepress' ); ?></p>
					<table class="form-table">
						<tbody id="items">
						<tr>
							<td>
		...
		?>
			<div class="postbox">
				<h3 class="hndle" style='cursor:auto;'><span><?php _e( 'Basic Certificate E-mail', 'coursepress' ); ?></span></h3>

				<div class="inside">
					<p class="description"><?php _e( 'E-mail to send certificate to student upon course completion. (if enabled)', 'coursepress' ); ?></p>
					<table class="form-table">
						<tbody id="items">
						<tr>
							<th><?php _e( 'From Name', 'coursepress' ); ?></th>
							<td>
						...
						<tr>
							<th><?php _e( 'From E-mail', 'coursepress' ); ?></th>
							<td>
						...
						<tr>
							<th><?php _e( 'E-mail Subject', 'coursepress' ); ?></th>
							<td>
						...
						<tr>
							<th><?php _e( 'E-mail Content', 'coursepress' ); ?></th>
							<td>
								<p class="description"><?php _e( 'These codes will be replaced with actual data: FIRST_NAME, LAST_NAME, COMPLETION_DATE, CERTIFICATE_NUMBER, UNIT_LIST, COURSE_NAME, COURSE_EXCERPT, COURSE_ADDRESS, WEBSITE_ADDRESS, WEBSITE_NAME', 'coursepress' ); ?></p>
								<?php
		...
		public static function make_pdf( $student, $course, $url = false ) {
			...
			$certificate_title = sprintf( __('Certificate %s', 'coursepress' ), $course_completed_details['certificate_number'] );
		...
		private static function _default_certificate_content() {
			...
			$default_certification_content = sprintf( __( '%1$s %2$s
				...
				Date: %4$s
				Certificate no.: %5$s', 'coursepress' ), $fields[0], $fields[1], $fields[2], $fields[3], $fields[4] );
			return $default_certification_content;
		}

		private static function _default_email_subject() {
			return sprintf( __( '[%s] Congratulations. You passed your course.', 'coursepress' ), get_option( 'blogname' ) );
		}
		...
		private static function _default_email_content() {
			...
			$default_instructor_invitation_email = sprintf( __(
				...
				, 'coursepress' ), 'FIRST_NAME', 'COURSE_NAME'
			);
