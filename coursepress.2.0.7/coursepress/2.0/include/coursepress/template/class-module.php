...
class CoursePress_Template_Module {
				...
				if ( empty( $attributes['allow_retries'] ) ) {
					...
					if ( $response_count >= $attempts ) {
						...
						$retry .= sprintf(
							...
							sprintf( __( 'Number of failed attempts: %d', 'cp' ), $response_count )
						);
					}
				}
	...
	private static function render_module_head( $module, $attributes = false ) {
		...
		if ( $show_title ) {
			...
			if ( $mandatory ) {
				$content .= sprintf( '<span class="is-mandatory">%s</span>', __( 'Required', 'cp' ) );
			}
			$content .= '</h4>';
		} else if ( $mandatory ) {
	...
	public static function form_result_content( $student_id, $course_id, $unit_id, $module_id, $form_result = false ) {
		...
		$passed_message = ! empty( $form_result['passed'] ) ? __( 'You have successfully passed the form. Here are your results.', 'cp' ) : __( 'You did not pass the form this time. Here are your results.', 'cp' );
		$template = '<div class="module-form-questions">
			<div class="coursepress-form-results ' . esc_attr( $passed_class ) . '">
				...
				<div class="form-results">
					<table>
					<tr><th>' . esc_html__( 'Total Questions', 'cp' ) . '</th><td>' . esc_html( $form_result['total_questions'] ) . '</td></tr>
					<tr><th>' . esc_html__( 'Correct', 'cp' ) . '</th><td>' . esc_html( $form_result['correct'] ) . '</td></tr>
					<tr><th>' . esc_html__( 'Incorrect', 'cp' ) . '</th><td>' . esc_html( $form_result['wrong'] ) . '</td></tr>
					<tr><th>' . esc_html__( 'Grade', 'cp' ) . '</th><td>' . esc_html( $form_result['grade'] ) . '%</td></tr>
					</table>
				</div>
			</div>
		</div>';
